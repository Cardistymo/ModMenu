...
public class FabricMod implements Mod {
	...
	public @NotNull Map<String, Collection<String>> getContributors() {
		...
		for (Person contributor : this.metadata.getContributors()) {
			contributors.put(contributor.getName(), Collections.singletonList("Contributor"));
		}
	...
	public @Nullable String getSha512Hash() throws IOException {
		if (!container.getContainingMod().isPresent() && container.getOrigin().getKind() == ModOrigin.Kind.PATH) {
			...
			Optional<Path> fileOptional = paths.stream()
	...
	static class ModMenuData {
		...
		public ModMenuData(Set<String> badges, Optional<String> parent, @Nullable DummyParentData dummyParentData, String id) {
